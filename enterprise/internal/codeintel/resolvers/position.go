...
type PositionAdjuster interface {
	...
	AdjustPosition(ctx context.Context, commit, path string, px clienttypes.Position, reverse bool) (string, clienttypes.Position, bool, error)
	...
	// are swapped.
	AdjustRange(ctx context.Context, commit, path string, rx clienttypes.Range, reverse bool) (string, clienttypes.Range, bool, error)
}
...
// indicating that the translation was successful. If revese is true, then the source and
// target commits are swapped.
func (p *positionAdjuster) AdjustPosition(ctx context.Context, commit, path string, px clienttypes.Position, reverse bool) (string, clienttypes.Position, bool, error) {
	hunks, err := p.readHunksCached(ctx, p.repo, p.commit, commit, path, reverse)
	if err != nil {
		return "", clienttypes.Position{}, false, err
	}
...
// that the translation was successful. If revese is true, then the source and target commits
// are swapped.
func (p *positionAdjuster) AdjustRange(ctx context.Context, commit, path string, rx clienttypes.Range, reverse bool) (string, clienttypes.Range, bool, error) {
	hunks, err := p.readHunksCached(ctx, p.repo, p.commit, commit, path, reverse)
	if err != nil {
...
func adjustPosition(hunks []*diff.Hunk, pos clienttypes.Position) (clienttypes.Position, bool) {
	line, ok := adjustLine(hunks, pos.Line)
	if !ok {
		return clienttypes.Position{}, false
	}

	return clienttypes.Position{Line: line, Character: pos.Character}, true
}
...
func adjustRange(hunks []*diff.Hunk, r clienttypes.Range) (clienttypes.Range, bool) {
	start, ok := adjustPosition(hunks, r.Start)
	if !ok {
		return clienttypes.Range{}, false
	}
	...
	return clienttypes.Range{Start: start, End: end}, true
}
